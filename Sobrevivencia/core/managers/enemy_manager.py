...
if __package__:
    ...
    from ..inimigos.scaling import active_spitter_cap, enemy_difficulty_rating, enemy_spawn_scales, highest_player_level, player_power_rating, player_power_rating_for
    ...
    from Sobrevivencia.core.inimigos.scaling import active_spitter_cap, enemy_difficulty_rating, enemy_spawn_scales, highest_player_level, player_power_rating, player_power_rating_for
...
class EnemyManager:
    ...
    def _miniboss_velocity(self, enemy, direction, chase_speed, dt):
        ...
        if enemy.action:
            ...
            if enemy.action_timer <= 0:
                if enemy.action == "leap_warn":
                    ...
                    enemy.pos = self.world.move_circle(enemy.pos, enemy.radius, direction_charge * 420, include_destructibles=False)
                    self._apply_laser_damage(start, end, 74, 24, ignore_enemy=enemy)
                    self.emit_particles(enemy.pos, count=22, color=COLORS["danger"], speed=210)
        ...
        if enemy.special_timer <= 0 and enemy.pos.distance_squared_to(target_mb.pos) < (900 * 900):
            ...
            if roll < 0.30 and enemy.summon_cooldown <= 0:
                ...
                self._start_miniboss_charge(enemy)
            else:
    ...
    def _start_miniboss_charge(self, enemy):
        ...
        self.item_events.append({"type": "danger_line", "start": Vector2(enemy.pos), "end": Vector2(target.pos), "width": 70, "age": 0.0, "duration": 0.55, "color": COLORS["danger"]})
        self.message = "Investida do mini-boss: saia da linha!"
